...
{
    printf("Usage: firmware_update [-b] <address> <hex file>\n");
    printf("  -b: update the bootloader instead of the main firmware\n");
    printf("  address: the board address (0-7)\n");
    printf("  hex file: the name of the hex file containing the new firmware\n");
}
...
uint16_t process_hex_line(char* string, uint8_t* buffer, uint16_t buflen, uint8_t* pType, uint32_t* pAddress)
{
    uint8_t rec_count;
...
int update_firmware(int address, char* filename)
{
    ...
    if (mcc118_firmware_version(address, &fw_version, &boot_version) != RESULT_SUCCESS)
    {
        printf("Error getting the firmware version.\n");
        return 1;
    }
    
    printf("Found device\nFirmware version %X.%02X\nBootloader version %X.%02X\n", 
    ...
    {
        if ((count = process_hex_line(string, buffer, 128, &rec_type, &flash_address)) == 0)
            continue;
    ...
    printf("Verifying...");
    if (mcc118_bl_read_crc(address, USER_START, USER_LENGTH, &crc) != RESULT_SUCCESS)
    {
        printf("error\n");
        mcc118_close(address);
        return 1;
...
int update_bootloader(int address, char* filename)
{
    ...
    for (flash_address = BOOT_START; flash_address < USER_START; flash_address += 128)
    {
        if (mcc118_bootmem_write(address, flash_address, 128, &virtual_flash[flash_address]) != RESULT_SUCCESS)
        {
            printf("error\n");
            mcc118_close(address);
            return 1;
    ...
    for (flash_address = BOOT_START; flash_address < USER_START; flash_address += 256)
    {
        if (mcc118_bootmem_read(address, flash_address, 256, buffer) != RESULT_SUCCESS)
        {
            printf("error\n");
            mcc118_close(address);
            return 1;
