...
uint8_t open_first_hat_device()
{
    ...
    // Get the number of MCC hat devices so that we can determine
    ...
    if (hat_count > 0)
    {
        ...
        hat_info_list = (struct HatInfo*)malloc(hat_count * sizeof(struct HatInfo));

        // Get list of MCC 118s.
        hat_list(HAT_ID_MCC_118, hat_info_list);

        // This application will use the first hat device (i.e. hat_info_list[0])
...
/***********************************************************************************
    ...
    Description:                Allocate arrays for the indicies and data for each
                                channel in the scan.
...
***********************************************************************************/
int allocate_channel_xy_arrays(uint8_t current_channel_mask, uint32_t numSamplesPerChannel)
{
    int i = 0;
    int chanMask = 0;
    int channel = 0;
    ...
    for (i=0; i< MAX_118_CHANNELS; i++)
    {
        ...
        {
            gtk_databox_graph_remove (GTK_DATABOX(box), graphChannelInfo[i].graph);
    ...
    for (i=0; i< MAX_118_CHANNELS; i++)
    {
        // If this channel is in the scan, then allocate the arrays for the indicies (X) and data (Y)
        if (chanMask & 1)
        {
...
/***********************************************************************************
    ...
    Parameters:
        ...
        stride                  The distance (in samples) between samples for this
                                channel.
        buffer_size_samples     The total number of samles in the buffer (for all
                                channels).
        first_block             Flag to indicate if this is the fist block and if so,
                                initialize the indicies (X array) for the channel.

    Return Value:
        NONE
***********************************************************************************/
void copy_data_to_xy_arrays(double* hat_read_buf, int read_buf_start_index, int channel, int stride, int buffer_size_samples, gboolean first_block)
{
    ...
    // For the first block, set the indicies and data.  For all other blocks,
    // just set the data
    if (first_block)
    {
        // Set indicies and data
...
void analog_in_finite ()
{
    ...
    // Allocate arrays for the indicies and data for each channel in the scan.
    int num_channels = allocate_channel_xy_arrays(channel_mask, iNumSamplesPerChannel);

    // set the timeout
    double scan_timeout = num_channels * iNumSamplesPerChannel / iRatePerChannel * 10; /*set duration of timeout */
    ...
    do
    {
        retval =(mcc118_a_in_scan_read(address, &read_status, 0, scan_timeout, hat_read_buf, buffer_size_samples, &samples_read_per_channel)); //was the scan started?
        if (retval != RESULT_SUCCESS)
        {
            show_mcc118_error(retval);
            break;
        }
    } while ((retval == RESULT_SUCCESS) && (read_status & STATUS_RUNNING) != STATUS_RUNNING);

    if (retval == RESULT_SUCCESS)
    {
        // Read all the samples requested
        retval =(mcc118_a_in_scan_read(address, &read_status, iNumSamplesPerChannel, scan_timeout, hat_read_buf, buffer_size_samples, &samples_read_per_channel));
        if (retval != RESULT_SUCCESS)
        {
            show_mcc118_error(retval);
        }
    ...
    if (retval == RESULT_SUCCESS)
    {
        ...
        retval = write_log_file(log_file_ptr, hat_read_buf, iNumSamplesPerChannel, num_channels);
        if (retval < 0)
        {
            switch (retval)
            {
    ...
    if (retval > 0)
    {
        ...
        while (chan_mask > 0)
        {
            ...
            if (chan_mask & 1)
            {
                ...
                copy_data_to_xy_arrays(hat_read_buf, read_buf_index, channel, num_channels, buffer_size_samples, TRUE);

                // Graph the data
                gfloat* X = graphChannelInfo[channel].X;
                gfloat* Y = graphChannelInfo[channel].Y;

                graphChannelInfo[channel].graph = gtk_databox_lines_new ((guint)iNumSamplesPerChannel, X, Y, graphChannelInfo[channel].color, 2);
                gtk_databox_graph_add (GTK_DATABOX (box), GTK_DATABOX_GRAPH(graphChannelInfo[channel].graph));
...
t2 = g_timer_elapsed(timer, &msec);
g_print ("analog_in_continuous(read data):  t1 = %lf, t2 = %lf, elapsed time = %lf\n", t1, t2, t2 - t1);
g_print ("analog_in_continuous(read data):  timeout = %f, samples read = %d\n", scan_timeout, samples_read_per_channel);

        // Write the data to a log file as CSV data
        retval = write_log_file(log_file_ptr, hat_read_buf, samples_read_per_channel, num_channels);
        if (retval < 0)
        {
            int error_code;
            switch (retval)
            ...
            g_main_context_invoke (context, (GSourceFunc)show_mcc118_error_main_thread, &error_code);
        ...
        while (chanMask > 0)
        {
            ...
            if (chanMask & 1)
            {
                if (first_block)
                {
                    // If this is the first block we need to set the indicies and the data
                    copy_data_to_xy_arrays(hat_read_buf, read_buf_index, channel, num_channels, buffer_size_samples, TRUE);
                    ...
                    graphChannelInfo[channel].graph = gtk_databox_lines_new ((guint)iNumSamplesPerChannel, X, Y, graphChannelInfo[channel].color, 2);
                    gtk_databox_graph_add (GTK_DATABOX (box), GTK_DATABOX_GRAPH(graphChannelInfo[channel].graph));
                    gtk_databox_set_total_limits (GTK_DATABOX (box), 0.0, (gfloat)iNumSamplesPerChannel, 11.0, -11.0);
                }
                else
                {
                    // If this is not the first block, just update the data
                    copy_data_to_xy_arrays(hat_read_buf, read_buf_index, channel, num_channels, buffer_size_samples, FALSE);
                }

                // Set the index to start at the first sample of the next channel
                read_buf_index++;
            }
            channel++;
            chanMask >>= 1;
        ...
        // Set the condition to cause the display update function to update the display
...
t2 = g_timer_elapsed(timer, &msec);
g_print ("analog_in_continuous:  t1 = %lf, t2 = %lf, elapsed time = %lf\n", t1, t2, t2 - t1);
...
/***********************************************************************************
    ...
    Description:                Event handler for the Start/Stop button.
                                ...
                                parse the UI settings before starting the acquisition.
...
void start_stop_event_handler(GtkWidget *widget, gpointer data)
{
    ...
    if (strcmp(StartStopBtnLbl , "Start") == 0)
    {
        ...
        iRatePerChannel =  gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON(spinRate));  //1000;  //
        ...
        gboolean bContinuous =  gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON (rbContinuous));
        if (bContinuous == TRUE)
...
g_print ("Scan options selected:  %d\n", options);
        ...
        retval = mcc118_a_in_scan_start(address, channel_mask , iNumSamplesPerChannel, iRatePerChannel, options); //OPTS_CONTINUOUS);  //
        if (retval != RESULT_SUCCESS)
        {
            show_mcc118_error(retval);
        ...
        {
            // If continuous scan, then start a thread to read the data from the device
        ...
        {
            // Its a finite scan, so call function to wait for all of the samples to be acquired
            analog_in_finite();
        }
...
/***********************************************************************************
    ...
    Parameters:
        ...
        data                    A pointer to user data for the event handler (contains
                                a pointer to the initial CSV file name)
...
//    GtkWidget *vboxMain, *vboxConfig, *label;
    ...
    GtkWidget *hboxChannel, *vboxChannel, *vboxLegend, *hboxNumSamples, *hboxRate;
    ...
    for (i = 0; i < MAX_118_CHANNELS; i++)
    {
        ...
        gtk_widget_override_background_color (legend[i], GTK_STATE_FLAG_NORMAL, &legendColor[i]);
    }
    ...
    gtk_radio_button_join_group ((GtkRadioButton*)rbFinite, (GtkRadioButton*)rbContinuous);

    btnSelectLogFile = gtk_button_new_with_label ( "Select Log File ...");
    g_signal_connect (btnSelectLogFile, "clicked", G_CALLBACK (select_log_file_event_handler), csv_filename);
    gtk_box_pack_start(GTK_BOX(vboxConfig), btnSelectLogFile,0,0,5);

    btnStart_Stop = gtk_button_new_with_label( "Start");
    g_signal_connect (btnStart_Stop, "clicked", G_CALLBACK (start_stop_event_handler), NULL);
...
int main (int argc, char *argv[])
{
    ...
    strcpy(application_name, "MCC188 Data Logger");
    ...
    // Create the application structure and set an event handler for the activate event.
    app = gtk_application_new ("org.mcc.example", G_APPLICATION_FLAGS_NONE);
    g_signal_connect (app, "activate", G_CALLBACK (activate_event_handler), NULL);
