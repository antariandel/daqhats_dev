...
*
*   Performs an ADC self offset correction. This is automatically run when
*   opening the board. For absolute accuracy this should be peformed whenever
*   the board temperature changes by more than 5 degrees C from the last time
*   the offset error was cleared. The board temperature can be monitored with
*   mcc134_cjc_read(). This function takes approximately 800ms to complete.
*
*   @param address  The board address (0 - 7). Board must already be opened.
*   @return [Result code](@ref ResultCode),
*       [RESULT_SUCCESS](@ref RESULT_SUCCESS) if successful.
...
*   returned temperature can have some special values to indicate abnormal
*   conditions:
...
*   
*   Returns the temperature of the channel terminal for cold junction
*   compensation.
*
*   @param address  The board address (0 - 7). Board must already be opened.
*   @param channel  The analog input channel number, 0 - 3.
*   @param cjc_temp   Receives the cold junction compensation temperature in
*       degrees C.
*   @return [Result code](@ref ResultCode),
*       [RESULT_SUCCESS](@ref RESULT_SUCCESS) if successful.
*/
